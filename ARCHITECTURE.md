...
- **Budget de tours par session** (`lib/turnbudget.js`, lot #124) : traduit le `turnCount` déjà
  persisté par `turnstats` en **palier de tours** franchi. Deux seuils, exactement les clés que la
  spec d'origine portait sans les exécuter — `budget.warn_after_session_turns` (défaut **12**) et
  `budget.recommend_fresh_session_after_turns` (défaut **20**), lues par `lib/rules.js`, bornes
  3–500 —, puis rappel **flottant tous les +10 tours** (même esprit que `occupancy.FLOATING_STEP`).
  **Actif par défaut** : les défauts sont codés en dur, `rules.yaml` ne fait que les déplacer (à la
  différence de la borne d'occupation du lot #112, qui est opt-in) ; l'unique interrupteur est la clé
  **non numérique** `budget.turn_budget: off` — aucun nombre ne peut éteindre le garde-fou, un `0`
  étant hors bornes donc ignoré donc ramené au défaut. Palier **monotone** persisté (mêmes primitives
  que le palier d'occupation), avec **un état par canal** (`<sha1>-turnbudget`,
  `<sha1>-turnbudget-prompt`) : sans cette séparation, le premier hook du tour à franchir le seuil
  consommerait l'anti-spam de l'autre. Deux canaux, deux rôles :
  - `stop.js` : rappel ⚠ dans les `parts` (constat, soumis à l'arbitre) au 1er seuil ; prescription
    ⛔ en **coda hors arbitre** au 2nd (nomme le lot en cours, propose le commit intermédiaire +
    session fraîche). Évalué **en toute fin de tour** et **muet si la coda de clôture est déjà
    posée** — le lot vient d'être clos, la reco de session fraîche y est déjà.
  - `user-prompt-submit.js` : injection `additionalContext` **au seul palier de prescription**,
    1×/palier. C'est le **canal décisif** : un `systemMessage` est visible par l'utilisateur mais
    jamais réinjecté au modèle, donc Claude ne le lit pas et continue d'étirer la session. Le
    compteur y est lu **sans incrément** (`turnstats.readTurnCount`) — il n'avance qu'au Stop.
  Argument chiffré, calculé et non recopié (`messages.turnSplitGainRange`) : coût ∝ tours^1,66 à
  ^1,98 → scinder une session coûte ~37 à 49 % de moins à travail égal. Fail-open partout.
- **Frein sur la sortie relue** (`lib/outputbudget.js`, lot #125) : le budget de tours mesure le
  *nombre* de tours, pas ce qu'ils *coûtent* à relire — une sortie longue (dumps, réponses
  verbeuses) est réinjectée dans le contexte à chaque tour suivant via le cache. Même gabarit que
  `turnbudget.js` (seuils `rules.readNumber` avec bornes + défauts en dur — `warn_after_output_tokens`
  **30000**, `warn_after_output_per_turn` **1500** — extinction par clé non numérique
  `budget.output_budget: off`) mais **un seul palier, un seul nudge par session** (pas de rappel
  flottant : le message porte sur un style à corriger, pas une action de clôture à répéter). Les
  deux seuils (cumul ET moyenne/tour, lus via `turnstats.readOutputStats`) doivent être franchis
  ensemble. **Plancher `MIN_MEASURED_TURNS` (3)** : sous ce plancher, un seul tour massif suffit à
  franchir les deux seuils et fait doublon avec `costlyTurnMessage` — les deux se disputaient une
  place de l'arbitre de tour (plafond 3, `lib/arbiter.js`), évinçant parfois un diagnostic distinct
  (repéré en bout-en-bout : le nudge évinçait l'alerte de coût par lot du lot #43 sur un scénario
  existant). Nudge ⚠ dans les `parts` (soumis à l'arbitre, ce n'est pas une prescription de
  clôture) — à la différence du budget de tours, jamais en coda. Fail-open partout, indépendant
  du projet pour la lecture (transcript + état seuls).
...
- **Occupation-tokens ET compteur de tours** — **révision (lot #124, 2026-07-29)** de la décision
  d'origine « occupation-tokens *plutôt que* compteur de tours (vs spec) ». Ce qui restait vrai : la
  méthode d'occupation est un signal réel, déjà éprouvé par `context-guard.py`, et PMZ la reprend à
  son compte. Ce que la mesure a démenti : qu'elle SUFFISE. Les dashboards de deux projets donnent
  une occupation médiane basse (~125k, ~15 % de la borne) — la borne d'occupation ne mord donc
  quasiment jamais — alors que la **loi d'échelle** mesurée par le dépôt sur ses propres sessions
  (cf. « loi d'échelle » plus haut, exposant > 1 : ≈ 1,2 sur le coût, ≈ 1,5 sur le cache-read ;
  1,66 à 1,98 sur les corpus les plus longs) dit que le coût est commandé par le **nombre de tours**.
  Les deux signaux ne mesurent donc pas la même chose et cohabitent : l'occupation dit la
  **saturation** du contexte (filet haut, zone rouge, auto-compact), les tours disent le **coût
  cumulé** (`lib/turnbudget.js`). Le compteur de tours n'était d'ailleurs pas à créer : `turnstats`
  le persistait déjà depuis le lot #8 sans que rien ne le lise.
