...
1. Vérifie l'état : `node ~/.config/opencode/pmz/scripts/detect-project.js`
...
3. Crée et complète le socle (jamais d'écrasement, repo git uniquement, jamais le code
   applicatif) : `node ~/.config/opencode/pmz/scripts/bootstrap-project.js --augment`
...
6. **Uniquement si le socle vient d'être CRÉÉ** (nouveau projet — pas `augmented`, un projet
   ...
   possible) —
   `node ~/.config/opencode/pmz/scripts/backlog.js trigram --suggest` liste 3 propositions.
   Une fois validé : `node ~/.config/opencode/pmz/scripts/backlog.js trigram --set XXX`
