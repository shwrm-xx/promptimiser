...
`node ~/.claude/promptimizer/scripts/archive.js index`
...
`node ~/.claude/promptimizer/scripts/archive.js show --id N`
...
`node ~/.claude/promptimizer/scripts/archive.js raw --id N`
...
- Fiche : `node ~/.claude/promptimizer/scripts/archive.js write --id N --stdin` (le corps
  doit commencer par `<!-- pmz:archive:lot N -->` ; borne dure 8 000 caractères ; une
  fiche existante n'est jamais écrasée sans `--force`).
- Brut : `node ~/.claude/promptimizer/scripts/archive.js raw --id N --stdin`.
- Rétro-remplissage tier 0 : `node ~/.claude/promptimizer/scripts/archive.js backfill --dry-run`
  puis sans `--dry-run`.
