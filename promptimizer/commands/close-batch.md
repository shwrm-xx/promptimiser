...
Point de départ : `node ${CLAUDE_PLUGIN_ROOT}/scripts/close-batch.js`
...
6bis. Rédiger la **fiche d'archive** du lot et l'écrire :
   `node ${CLAUDE_PLUGIN_ROOT}/scripts/archive.js write --id N --stdin` (squelette pré-rempli
...
6ter. Archiver le handoff manuel **avant** qu'il ne soit détruit — la commande exacte (avec le
   ...
   `node ${CLAUDE_PLUGIN_ROOT}/scripts/archive.js raw --id N --file .vibe-agent/handoff.md`.
