...
3. Si un epic a été validé, l'enregistrer une fois pour la session/le titre :
   `node ${CLAUDE_PLUGIN_ROOT}/scripts/backlog.js epic --set "Nom de l'epic"`
...
4. Persister chaque lot validé (un appel par lot), avec sa préconisation de modèle, son
   ...
   `node ${CLAUDE_PLUGIN_ROOT}/scripts/backlog.js add --title "…" --scope "fait quand : …" --model sonnet --effort medium --verify "npm test" --epic "Nom de l'epic" --perimeter "src/a/**" --depends 12 --us "docs/us/US-42.md"`
...
4bis. **Si ≥ 2 lots viennent d'être persistés**, calculer le plan de vagues :
   `node ${CLAUDE_PLUGIN_ROOT}/scripts/backlog.js parallelize --json` (ajouter `--epic "…"` si
...
5. Démarrer et traiter le lot voulu : `node ${CLAUDE_PLUGIN_ROOT}/scripts/backlog.js start --id <id>`,
