#!/bin/bash
# Diagnostic Vibe Session Governor.
set -u

DEST="$HOME/.claude"
SETTINGS="$DEST/settings.json"
VSG="$DEST/vibe-session-governor"
MS="$VSG/install/merge-settings.js"
NO_PAUSE="${1:-}"
...
# Hooks VSG + double Stop
HOOKS_OK="—"; DOUBLE="no"
if [ -f "$MS" ] && [ "$HAS_NODE" -eq 1 ]; then
  CHK="$(node "$MS" "$SETTINGS" --check 2>/dev/null)"
  echo "$CHK" | grep -q '"vsg_hooks_present": true' && HOOKS_OK="OK"
  echo "$CHK" | grep -q '"double_stop": true' && DOUBLE="yes"
fi

# Skill
[ -f "$DEST/skills/vibe-session-governor/SKILL.md" ] && SKILL_OK="OK" || SKILL_OK="—"

# Scripts + dry-run réel d'un hook
SCRIPTS_OK="—"
if [ "$HAS_NODE" -eq 1 ] && [ -f "$VSG/hooks/session-start.js" ]; then
  echo '{}' | node "$VSG/hooks/session-start.js" >/dev/null 2>&1 && SCRIPTS_OK="OK"
fi

# Projet courant
PROJ="non initialisé"
if [ "$HAS_NODE" -eq 1 ] && [ -f "$VSG/scripts/detect-project.js" ]; then
  DET="$(node "$VSG/scripts/detect-project.js" 2>/dev/null)"
...
echo "Vibe Session Governor — diagnostic"
echo
...
[ "$DOUBLE" = "yes" ] && echo "Avertissement : deux hooks Stop actifs (VSG + context-guard.py)."
